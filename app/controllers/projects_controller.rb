...
class ProjectsController < ApplicationController
  ...
  def create
    ...
    if @project.saved?
      redirect_to project_path(@project), notice: '项目被成功创建。'
    else
      render 'new'
    end
  end
  ...
  def update
    ...
    respond_to do |format|
      if status
        flash[:notice] = '项目已成功更新。'
        format.html { redirect_to edit_project_path(@project), notice: '项目已成功更新。' }
        format.js
      else
  ...
  def upload_image
    ...
    respond_to do |format|
      if link_to_image
        ...
        format.json { render json: '无效文件.', status: :unprocessable_entity }
      end
    end
  end
