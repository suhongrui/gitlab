...
class Admin::UsersController < Admin::ApplicationController
  ...
  def block
    if user.block
      redirect_to :back, alert: "成功阻止了"
    else
      redirect_to :back, alert: "错误发生。用户是不会阻塞"
    end
  end

  def unblock
    if user.activate
      redirect_to :back, alert: "成功解除阻塞"
    else
      redirect_to :back, alert: "错误发生。用户未解除阻塞"
    end
  end
  ...
  def update
    ...
    respond_to do |format|
      if user.update_attributes(user_params_with_pass)
        user.confirm!
        format.html { redirect_to [:admin, user], notice: '用户已成功更新。' }
        format.json { head :ok }
      else
  ...
  def remove_email
    ...
    respond_to do |format|
      format.html { redirect_to :back, notice: "成功删除电子邮件。" }
