...
class Project < ActiveRecord::Base
  ...
  validates :import_url,
    format: { with: URI::regexp(%w(git http https)), message: '应该是一个有效的URL' },
    if: :import?
  ...
  def check_limit
    unless creator.can_create_project? or namespace.kind == 'group'
      errors[:limit_reached] << ("你的项目的限制是 #{creator.projects_limit} projects! Please contact your administrator to increase it")
    end
  rescue
