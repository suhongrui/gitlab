...
class User < ActiveRecord::Base
  ...
  def namespace_uniq
    ...
    if existing_namespace && existing_namespace != self.namespace
      self.errors.add :username, "已经存在"
    end
  end

  def avatar_type
    unless self.avatar.image?
      self.errors.add :avatar, "只允许图片"
    end
  end

  def unique_email
    self.errors.add(:email, '已有人使用') if Email.exists?(email: self.email)
  end

  def owns_notification_email
    self.errors.add(:notification_email, "不是你自己的电子邮件") unless self.all_emails.include?(self.notification_email)
  end
  ...
  def post_create_hook
    log_info("用户 \"#{self.name}\" (#{self.email}) was created")
    notification_service.new_user(self, @reset_token) if self.created_by_id
    system_hook_service.execute_hooks_for(self, :create)
  end

  def post_destroy_hook
    log_info("用户 \"#{self.name}\" (#{self.email})  was removed")
